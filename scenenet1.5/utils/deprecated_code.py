...
def init_metrics(tau=0.65, ignore_index=-1):
    return MetricCollection([
        JaccardIndex(num_classes=2, threshold=tau, ignore_index=ignore_index),
        Precision(threshold=tau, ignore_index=ignore_index),
        Recall(threshold=tau, ignore_index=ignore_index),
        F1Score(threshold=tau, ignore_index=ignore_index),
        FBetaScore(beta=0.5, threshold=tau, ignore_index=ignore_index),
        #AveragePrecision(),
...
def load_state_dict(model_path, gnet_class, model_tag='loss', kernel_size=None) -> Union[None, Tuple[object, Mapping[str, Any]]]:
    """
