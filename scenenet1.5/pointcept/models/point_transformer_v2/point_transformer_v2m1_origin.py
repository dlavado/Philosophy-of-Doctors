...
class PointBatchNorm(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        print(f"norm input: {input.shape}")
...
class PointTransformerV2(nn.Module):
    ...
    def forward(self, data_dict):
        if isinstance(data_dict, dict):
            ...
            coord, feat, batch = self._process_input(data_dict)
            offset = batch2offset(batch)
...
if __name__ == '__main__':
    ...
    model = PointTransformerV2(in_channels, num_classses).cuda()
    model.train()
    # model = SpUNetNoSkipBase(in_channels, num_classses)

    # random pointcloud with just xyz with batch = b
    xyz = torch.rand(batch_size, 50000, 3).cuda()
