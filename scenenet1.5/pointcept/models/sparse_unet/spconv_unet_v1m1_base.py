...
class SpUNetBase(nn.Module):
    ...
    def _process_input(self, inpt:torch.Tensor):
        """
        ...
        if inpt.shape[-1] > 3:
            ...
            feat = torch.zeros(coords.shape[0], 1).to(coords.device)

        batch = torch.cat([torch.full((inpt.shape[1],), i, device=inpt.device) for i in range(inpt.shape[0])], dim=0).to(coords.device)
    ...
    def forward(self, input_dict):
        ...
        if isinstance(input_dict, dict):
            ...
            discrete_coord, feat, batch = self._process_input(input_dict)
        ...
        print(x.indices.shape, x.features.device)
        x = self.conv_input(x)
        skips = [x]
        # enc forward
        for s in range(self.num_stages):
...
class SpUNetNoSkipBase(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        discrete_coord, feat, batch = self._process_input(data_dict)
        print(discrete_coord.shape, feat.shape, batch.shape)
...
if __name__ == '__main__':
    ...
    model = SpUNetBase(in_channels, num_classses)
    # model = SpUNetNoSkipBase(in_channels, num_classses)

    # random pointcloud with just xyz with batch = b
    xyz = torch.rand(batch_size, 5000, 3)

    # random pointcloud with xyz and features with batch = b
    xyz_feat = torch.rand(batch_size, 5000, 3 + 1)
