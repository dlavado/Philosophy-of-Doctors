...
class Farthest_Point_Sampling:
    ...
    def __init__(self, num_points: Union[int, torch.Tensor], radius=0.1, min_density_threshold=10) -> None:
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # pointcloud, _ = self.sample_farthest_points_naive(data, lengths, K=self.num_points) # shape = (B, N, 3 + F)
