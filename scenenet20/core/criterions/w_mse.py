...
import utils.my_utils as mu


class WeightedMSE(torch.nn.Module):

    def __init__(self, targets=None, weighting_scheme_path=None, weight_alpha=1, weight_epsilon=0.1, mse_weight=1, **kwargs) -> None:
        """
        ...
        if weighting_scheme_path is not None and os.path.exists(weighting_scheme_path):
            ...
            self.freqs, self.ranges = mu.load_pickle(self.pik_name)
            ...
            mu.save_pickle((self.freqs, self.ranges), f"{os.path.join('.', 'hist_estimation.pickle')}")
        else:
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--hist_path', type=str, default=None, help='Path to a previously computed histogram estimation')
