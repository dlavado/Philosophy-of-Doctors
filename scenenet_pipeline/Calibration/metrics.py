...
Based on code made by Jonathan Pearce
...
class CalibrationBase():
    """
    Base builder to compute Calibration Error Measures
    """

    def compute_bin_boundaries(self, probabilities = np.array([])):
        """

        Computes the lower and upper boundaries for each bin.

        Parameters
        ----------

        `probabilities` - np.ndarray:
            array with output probabilities to use when we want to separate the bins by number of samples instead of uniformly
        """
    ...
    def get_probabilities(self, output, labels, logits):
        """
        Converts probabilities to admissible format and computes confidences and accuracies.

        Parameters
        ----------

        `output` - np.ndarray:
            matrix with shape (N, C) where N is the number of samples and C is the number of classes with the output predictions

        `labels` - np.ndarray:
            array with shape (N,) that contains the ground truth labels for the output predictions.

        `logits` - bool:
            in case the predictions in `output` do not correspond to actual probabilities, we apply the softmax function
        """
        ...
        self.predictions = self.confidences > 0
        self.accuracies = np.equal(self.predictions, self.labels > 0)
...
class MaxProbCELoss(CalibrationBase):
    def loss(self, output, labels, n_bins = 15, logits = False, tau=0.7):
        """
        Parameters
        ----------
        ...
        `n_bins` - int:
            number of bins to use in the calibration diagram
...
class SCELoss(CalibrationBase):
    ...
    def loss(self, output, labels, n_bins = 15, logits = True):
        ...
        super().__init__(output, labels, n_bins, logits)
...
class TACELoss(CalibrationBase):
