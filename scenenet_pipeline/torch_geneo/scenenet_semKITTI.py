...
#EXT_DIR = "/home/d.lavado/" #cluster data dir
SEMK_DATA_PATH = os.path.join(EXT_DIR, 'SemKITTI/dataset')

SNET_PIPELINE_PATH = os.path.join(ROOT_PROJECT, 'scenenet_pipeline')

META_PATH = os.path.join(SNET_PIPELINE_PATH, 'snet_semK_models')
PICKLE_DIR = os.path.join(SNET_PIPELINE_PATH, "torch_geneo/models")
FREQ_SAMPLES = os.path.join(SNET_PIPELINE_PATH, "dataset/freq_samples")

TAU=0.7

semk_splits = ['samples', 'train', 'val', 'test']
...
def arg_parser():
    ...
    parser.add_argument("--model_date", type=str, default='today') # use 'saved' to use good models

    # Training Config
    parser.add_argument("--semK_split", type=str, choices=semk_splits, default='train')
...
def transform_batch(batch):
    vxg, gt = batch
    return vxg.to(device), gt.to(device)
...
def process_batch(gnet:SCENE_Net, batch, geneo_loss:GENEO_Loss, opt : Union[torch.optim.Optimizer, None], metrics:Union[MetricCollection, None], requires_grad=True):
    ...
    if metrics is not None:
        ...
        metrics(torch.flatten(pred), torch.flatten(batch[-1]).to(torch.int))
    return loss, pred
...
def train_observer(tau=TAU):
    ...
    print("Loading semK dataset...")
    semK_train_loader = DataLoader(semK_train, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)

    semK_val_loader = DataLoader(semK_val, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)
    ...
    geneo_loss = gnet_loss(torch.tensor([]), hist_path=PICKLE_DIR, alpha=ALPHA, rho=RHO, epsilon=EPSILON)
    

    # --- Train GENEO Net ---
    plot_metrics, gnet_params, best_metrics = training_loop(gnet, semK_train_loader, semK_val_loader, geneo_loss, opt, tau)
...
def examine_samples(model_path, data_path, tau=None, tag='loss'):
    ...
    geneo_loss = GENEO_Loss(torch.tensor([]), hist_path=PICKLE_DIR, alpha=ALPHA, rho=RHO, epsilon=EPSILON)
    ...
    test_loss = test_loss /  len(semK_loader)
    test_res = test_metrics.compute()
    print(f"\ttest_loss = {test_loss:.3f};")
    for met in test_res:
        print(f"\t{met} = {test_res[met]:.3f};")
...
if __name__ == "__main__":
    ...
    if parser.model_date == 'saved':
        META_TODAY = os.path.join(META_PATH, 'models_geneo')
    elif parser.model_date == 'today':
        META_TODAY = os.path.join(META_PATH, str(datetime.now().date()))
    else:
        date = parser.model_date.split('-') if '-' in parser.model_date else parser.model_date.split('/')
        date = list(map(int, date))
        META_TODAY = os.path.join(META_PATH, str(datetime(*date).date()))
    ...
    if parser.tuning:
        ...
        aux_date_path = os.path.join(META_PATH, str(datetime.now().date())) # corresponds to META_TODAY
    ...
    # retrieving label of object of interest
    ...
    semK_train = semKITTIv2(dataset_path=SEMK_DATA_PATH, split=parser.semK_split, transform=composed)
    
    semK_val = semKITTIv2(dataset_path=SEMK_DATA_PATH, split='val', transform=composed)

    semK_test = semKITTIv2(dataset_path=SEMK_DATA_PATH, split='test', transform=composed)

    # --- HyperParameters ---
    NUM_EPOCHS = parser.epochs
    BATCH_SIZE = parser.batch_size
    NUM_SAMPLES = len(semK_train)
    ...
    #vxlgrid_size = (64, 64, 64)

    kernel_size = (8, 8 ,8) if parser.k_size is None else tuple(*parser.k_size)
