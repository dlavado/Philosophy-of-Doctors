...
class GENEO_Loss(WeightedMSE):
    ...
    def arrow_regularizer(self, params:torch.nn.ParameterDict):

        geneo_names = [key for key in params.keys() if 'cone_' in key and 'geneo' in key]
        geneo_names = np.array(['cone_' + key.split('_')[2] for key in geneo_names])
        geneo_names = np.unique(geneo_names)

        return sum([self._flag_constraint(params[f'geneos_{geneo}_geneo_params_cone_radius'] - 2*params[f'geneos_{geneo}_geneo_params_radius']) 
                for geneo in geneo_names])
    
    def cylinder_regularizer(self, params:torch.nn.ParameterDict):

        geneo_names = [key for key in params.keys() if 'cy_' in key and 'geneo' in key]
        geneo_names = np.array(['cy_' + key.split('_')[2] for key in geneo_names])
        geneo_names = np.unique(geneo_names)

        return sum([self._flag_constraint(9/2 - params[f"geneos_{geneo}_geneo_params_radius"]) for geneo in geneo_names])
