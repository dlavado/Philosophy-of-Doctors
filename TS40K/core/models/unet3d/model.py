...
from core.models.unet3d.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
    create_decoders, create_encoders
from core.models.unet3d.utils import get_class, number_of_features_per_level
...
class AbstractUNet(nn.Module):
    ...
    Args:
        ...
        is_geneo (bool): if True the model is 3D, otherwise 2D, default: True
    """

    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, is_segmentation=True, conv_kernel_size=3, pool_kernel_size=2,
                 conv_padding=1, conv_upscale=2, upsample='default', dropout_prob=0.1, is_geneo=True):
        ...
        self.encoders = create_encoders(in_channels, f_maps, basic_module, conv_kernel_size,
                                        ...
                                        layer_order, num_groups, pool_kernel_size, is_geneo)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        for decoder, encoder_features in zip(self.decoders, encoders_features):
            ...
            # print(f"decoder: {x.shape}; {x.requires_grad}; {x.grad_fn}")

        x = self.final_conv(x)

        # print(f"final_conv: {x.shape}; {x.requires_grad}; {x.grad_fn}")
        ...
        # print(f"final_activation: {x.shape}; {x.requires_grad}; {x.grad_fn}")
    ...
    def get_geneo_params(self):
        geneo_params = nn.ParameterDict()
        for module in self.encoders:
            geneo_params.update(module.get_geneo_params())
    ...
    def get_cvx_coefficients(self):
        cvx_coefficients = nn.ParameterDict()
        for module in self.encoders:
            cvx_coefficients.update(module.get_cvx_coefficients())
        
        for module in self.decoders:
            cvx_coefficients.update(module.get_cvx_coefficients())

        return cvx_coefficients
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 ...
                 conv_upscale=2, upsample='default', dropout_prob=0.1, is_geneo=True, **kwargs):
...
if __name__ == '__main__':
    ...
    # model = UNet3D(in_channels=1, out_channels=6, final_sigmoid=False, f_maps=64, layer_order='gcr', is_geneo=True).cuda()
    # model = model.eval()
    ...
    torchinfo.summary(model, input_size=(2, 1, 32, 32, 32))
