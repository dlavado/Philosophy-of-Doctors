...
from utils.scripts_utils import pointcloud_to_wandb
...
class LitSceneNet_multiclass(LitWrapperModel):
    ...
    def __init__(self, 
        ...
        model = SceneNet_multiclass(geneo_num, num_observers, extra_feature_dim, kernel_size, hidden_dims, classifier=classifier, num_points=num_points, num_classes=num_classes)
        super().__init__(model, criterion, optimizer, learning_rate, None)

        ig_index = 0
        
        if metric_initializer is not None:
            self.train_metrics:MetricCollection = metric_initializer(num_classes=num_classes, ignore_index=ig_index)
            self.val_metrics:MetricCollection = metric_initializer(num_classes=num_classes, ignore_index=ig_index)
            self.test_metrics:MetricCollection = metric_initializer(num_classes=num_classes, ignore_index=ig_index)
