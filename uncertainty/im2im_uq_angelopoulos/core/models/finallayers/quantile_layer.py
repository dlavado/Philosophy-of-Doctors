...
class QuantileRegressionLayer(nn.Module):
    def __init__(self, n_channels_middle, n_channels_out, params):
        ...
        self.lower = nn.Conv3d(n_channels_middle, n_channels_out, kernel_size=3, padding=1)
        self.prediction = nn.Conv3d(n_channels_middle, n_channels_out, kernel_size=3, padding=1)
        self.upper = nn.Conv3d(n_channels_middle, n_channels_out, kernel_size=3, padding=1)
