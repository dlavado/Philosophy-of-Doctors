...
###############################################################
# /home/didi/anaconda3/envs/im2im-uq/bin/pip install sklearn
...
class UNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, mode='nearest') -> None:
        ...
        self.down1 = DownScale(16, 32)
        self.down2 = DownScale(32, 64)
        self.down3 = DownScale(64, 128)
        factor = 2 if not mode == 'transpose' else 1
        self.down4 = DownScale(128, int(256 / factor))
        self.up1 = UpScale(256, int(128 / factor), mode)
        self.up2 = UpScale(128, int(64 / factor), mode)
        self.up3 = UpScale(64, int(32 / factor), mode)
        self.up4 = UpScale(32, 16, mode)
