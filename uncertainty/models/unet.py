...
class UNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, mode='nearest') -> None:
        ...
        self.n_channels_middle = n_channels
        self.n_channels_out = n_classes

        self.inc = ConvBlock(n_channels, 16*2)
        self.down1 = DownScale(16*2, 32*2)
        self.down2 = DownScale(32*2, 64*2)
        self.down3 = DownScale(64*2, 128*2)
        factor = 2 if not mode == 'transpose' else 1
        self.down4 = DownScale(128*2, int(256*2 / factor))
        self.up1 = UpScale(256*2, int(128*2 / factor), mode)
        self.up2 = UpScale(128*2, int(64*2 / factor), mode)
        self.up3 = UpScale(64*2, int(32*2 / factor), mode)
        self.up4 = UpScale(32*2, 16*2, mode)
    ...
    def __str__(self):
        print(f"{'='*20} UNet Summary {'='*20}")
        input_shape = (128,)*3
        batch_size=4
        print(f"Assuming input shape: {input_shape}, and batch size: {batch_size}")
        return summary(self, input_shape, batch_size=batch_size, device='cuda')
